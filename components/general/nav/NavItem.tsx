...
function NavItem({ children, text, disabled, route, onClick }: NavItemProps) {
    ...
    return (
        <>
        {route ?
            <Link onClick={() => onClick && onClick()} key={text} href={route} className={`flex group gap-2 relative items-center ${disabled ? 'text-stone-400 cursor-not-allowed' : 'text-stone-700 hover:text-emerald-500'}`}>
                {children}
